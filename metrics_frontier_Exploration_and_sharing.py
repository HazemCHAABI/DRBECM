...
# ============================
NUM_ROBOTS_LIST = range(11, 16)  # Number of robots from 11 to 16
...
class Robot:
    ...
    def sense_area(self):
        x, y = np.clip(np.round(self.p), 0, np.array(self.map.shape) - 1).astype(int)

        new_cells = 0
        i_range = np.arange(max(0, x - self.R_s), min(self.map.shape[1], x + self.R_s + 1))
        j_range = np.arange(max(0, y - self.R_s), min(self.map.shape[0], y + self.R_s + 1))
        ...
        for i in i_range:
            for j in j_range:
                if (i - x)**2 + (j - y)**2 <= self.R_s**2:
                    ...
                    self.map[j, i] = True  # Update the shared map
        return new_cells

    def update_frontier(self):
        y, x = np.nonzero(~self.map)
    ...
    def is_frontier(self, x, y):
        ...
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                if 0 <= y + dy < self.map.shape[0] and 0 <= x + dx < self.map.shape[1]:
...
class Environment:
    def __init__(self, width, height, num_robots):
        ...
        for i in range(num_robots):
            ...
            self.robots.append(Robot(i, x, y, self.global_map))
...
def run_simulation():
    ...
    with open('simulation_metrics_Only_FrontierSharing_M120_R100.csv', mode='w', newline='') as csv_file:
        ...
        for num_robots in NUM_ROBOTS_LIST:
            ...
            for run in range(NUM_RUNS):
                ...
                # Run the simulation for MAX_FRAMES or until coverage reaches 100%
                for frame in range(MAX_FRAMES):
                    env.update(dt=1)
                    coverage = env.global_map.sum() / (env.width * env.height)
                    if coverage == 1.0:  # Stop if coverage reaches 100%
                        break
                # Collect metrics
